...
def run_inference():
    weights_path = r"d:\PCB Defect project\DeepPCB_YOLO\runs\yolov12n_ghost_extreme_pcb\weights\best.pt"
    
    if not os.path.exists(weights_path):
        weights_path = r"d:\PCB Defect project\DeepPCB_YOLO\runs\yolov12n_ghost_extreme_pcb\weights\last.pt"
    
    if not os.path.exists(weights_path):
        print(f"Weights not found at {weights_path}.")
        print("Please ensure you have run 'train_yolov12_ghost_extreme.py' to train the model first.")
        return

    print(f"Loading YOLOv12-Extreme-Ghost model from {weights_path}...")
    try:
    ...
    output_dir = r"d:\PCB Defect project\DeepPCB_YOLO\inference_results_v12_ghost_extreme"
    ...
    for img_path in sample_images:
        ...
        for result in results:
            for box in result.boxes:
                ...
                cv2.putText(img, f"ExGhost: {CLASS_NAMES[cls_id]} {conf:.2f}", (x1, y2 + 15), 0, 0.5, (0, 0, 255), 1)

        base_name = os.path.basename(img_path)
        save_path = os.path.join(output_dir, "exghost_" + base_name)
